...
import pytest  # noqa
from upcheck.url_check import UrlCheck, UrlChecks
...
def test_read_config():
    ...
    checks = UrlChecks.create_checks(config_path)

    assert len(checks.url_checks) == 2

    for item in checks.url_checks:
        assert item.url
...
def test_read_invalid_yaml_config():
    ...
    with pytest.raises(Exception) as excinfo:
        UrlChecks.create_checks(config_path)
